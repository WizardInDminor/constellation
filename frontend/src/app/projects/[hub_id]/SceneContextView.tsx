...
export function SceneContextView({
  ...
  if (!ctx) {
    return <p className="text-xs text-gray-400 p-4">Loading scene context…</p>;
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr_280px] gap-3">
        ...
        <aside className="space-y-3 lg:max-h-[calc(100vh-12rem)] lg:overflow-y-auto">
          <ContextSection
            label="Characters"
            relevance="strong"
            items={characters}
        ...
        <main className="rounded-lg border border-gray-200 bg-white p-4 lg:max-h-[calc(100vh-12rem)] lg:overflow-y-auto">
          {focusedNodeId ? (
            <FocusedNodeView
...
function FocusedNodeView({
  ...
  if (!node) return <p className="text-xs text-gray-400">Loading…</p>;
...
function EmptyCenter({
  ...
  return (
    <div className="space-y-3 text-center text-xs text-gray-500">
      ...
      <p className="italic">
        Click any element on the left or right to drill down. Ctrl+click to
        edit inline.
      </p>
      {(preceding || following) && (
        <div className="text-[11px] text-gray-400 pt-3 border-t border-gray-100">
          {preceding && (
            <p>Preceding: <span className="text-gray-700">{preceding.title}</span></p>
          )}
          {following && (
            <p>Following: <span className="text-gray-700">{following.title}</span></p>
          )}
        </div>
      )}
    </div>
